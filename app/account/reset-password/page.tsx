...
export async function buildResetPasswordMetadata(locale: AppLocale): Promise<Metadata> {
  const t = await getScopedTranslator(locale, "ResetPasswordPage");
  const metadataCopy = getAccountRouteMetadataCopy(locale, "resetPassword");
...
export async function generateMetadata() {
  return buildResetPasswordMetadata(await resolveServerLocaleFallback());
}
...
export default async function ResetPasswordPage({
  ...
  return (
    <PageShell
      feedbackContext={{
        pageType: "other",
        pagePath: "/account/reset-password",
