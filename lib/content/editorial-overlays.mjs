...
const genericNonTranslatableKeys = new Set([
  "id",
  "slug",
  "metric",
  "param",
...
const genericNonTranslatableSegments = new Set([
  "conditions",
  "values",
  "defaults",
  "series",
...
function shouldSkipGenericTranslationPath(pathSegments) {
  const lastKey = pathSegments.at(-1);
  return (
    genericNonTranslatableKeys.has(lastKey) ||
    pathSegments.some((segment) => genericNonTranslatableSegments.has(segment))
  );
}

function buildGenericTranslatableOverlay(value, pathSegments = []) {
  if (typeof value === "string") {
    return shouldSkipGenericTranslationPath(pathSegments) ? undefined : value;
  }

  if (Array.isArray(value)) {
    if (shouldSkipGenericTranslationPath(pathSegments)) {
      return undefined;
    }
    ...
    if (stableKey) {
      const items = value
        .map((item) => {
          const translatedItem = buildGenericTranslatableOverlay(item, pathSegments);
          return isPlainObject(translatedItem) && Object.keys(translatedItem).length > 1
            ? translatedItem
            : undefined;
        })
        .filter((item) => item !== undefined);
    ...
    const items = value
      .map((item, index) => buildGenericTranslatableOverlay(item, [...pathSegments, String(index)]))
      .filter((item) => item !== undefined);
    return items.length > 0 ? items : undefined;
  }
  ...
  if (isPlainObject(value)) {
    ...
    for (const [key, nestedValue] of Object.entries(value)) {
      if ((key === "id" || key === "slug") && typeof nestedValue === "string") {
        translated[key] = nestedValue;
        continue;
      }

      const translatedValue = buildGenericTranslatableOverlay(nestedValue, [...pathSegments, key]);
...
function buildConceptEditorialOverlaySource(conceptMetadata, conceptContent) {
  ...
  const v2 = conceptContent?.v2;
  if (v2) {
    const localizedV2 = {};

    withOptional(localizedV2, "intuition", v2.intuition);
    withOptional(localizedV2, "whyItMatters", v2.whyItMatters);

    if (isPlainObject(v2.equationSnapshot)) {
      const localizedEquationSnapshot = {};
      withOptional(localizedEquationSnapshot, "title", v2.equationSnapshot.title);
      withOptional(localizedEquationSnapshot, "note", v2.equationSnapshot.note);

      if (Object.keys(localizedEquationSnapshot).length > 0) {
        localizedV2.equationSnapshot = localizedEquationSnapshot;
      }
    }

    if (isPlainObject(v2.startSetup) && typeof v2.startSetup.note === "string") {
      localizedV2.startSetup = { note: v2.startSetup.note };
    }

    if (Array.isArray(v2.guidedSteps) && v2.guidedSteps.length > 0) {
      localizedV2.guidedSteps = v2.guidedSteps.map((step) => {
        const localizedStep = { id: step.id };
        withOptional(localizedStep, "title", step.title);
        withOptional(localizedStep, "goal", step.goal);
        withOptional(localizedStep, "doThis", step.doThis);
        withOptional(localizedStep, "notice", step.notice);
        withOptional(localizedStep, "explain", step.explain);

        if (isPlainObject(step.setup) && typeof step.setup.note === "string") {
          localizedStep.setup = { note: step.setup.note };
        }

        if (isPlainObject(step.inlineCheck)) {
          const localizedInlineCheck = {};
          withOptional(localizedInlineCheck, "title", step.inlineCheck.title);
          withOptional(localizedInlineCheck, "note", step.inlineCheck.note);

          if (Object.keys(localizedInlineCheck).length > 0) {
            localizedStep.inlineCheck = localizedInlineCheck;
          }
        }
    ...
    if (isPlainObject(v2.wrapUp)) {
      const localizedWrapUp = {};
      withOptional(
        localizedWrapUp,
        "learned",
        v2.wrapUp.learned,
      ...
      withOptional(localizedWrapUp, "misconception", v2.wrapUp.misconception);
      withOptional(localizedWrapUp, "freePlayPrompt", v2.wrapUp.freePlayPrompt);

      if (Array.isArray(v2.wrapUp.nextConcepts) && v2.wrapUp.nextConcepts.length > 0) {
        localizedWrapUp.nextConcepts = v2.wrapUp.nextConcepts.map((concept) => {
          const localizedConcept = { slug: concept.slug };
          withOptional(localizedConcept, "title", concept.title);
          withOptional(localizedConcept, "reasonLabel", concept.reasonLabel);
          return localizedConcept;
        });
      }

      if (Object.keys(localizedWrapUp).length > 0) {
        localizedV2.wrapUp = localizedWrapUp;
      }
    }

    if (Object.keys(localizedV2).length > 0) {
      overlay.v2 = localizedV2;
    }
  }
  ...
  const genericOverlay = buildGenericTranslatableOverlay(conceptContent);
  return genericOverlay ? mergeEditorialValue(overlay, genericOverlay) : overlay;
}
